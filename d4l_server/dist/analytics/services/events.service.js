...
class EventsService {
     ...
     */
    async storeContractEvent(event) {
        ...
        `;
            ...
            if (!db_1.inMemoryStorage.events[event.eventType]) {
                db_1.inMemoryStorage.events[event.eventType] = [];
            }
            db_1.inMemoryStorage.events[event.eventType].push(eventWithId);
            return id;
        });
    }
    /**
    ...
    async queryEvents(params) {
        ...
        `;
            ...
            Object.values(db_1.inMemoryStorage.events).forEach(events => {
            ...
            if (params.contractAddress) {
                allEvents = allEvents.filter(event => event.contractAddress === params.contractAddress);
            }
            if (params.eventType) {
    ...
    async getEventById(id) {
        ...
        `;
            ...
            for (const eventType in db_1.inMemoryStorage.events) {
                const event = db_1.inMemoryStorage.events[eventType].find(e => e.id === id);
                if (event) {
                    return event;
                }
            }
    ...
    async deleteEventById(id) {
        ...
        `;
            ...
            let deleted = false;
            for (const eventType in db_1.inMemoryStorage.events) {
                const index = db_1.inMemoryStorage.events[eventType].findIndex(e => e.id === id);
                if (index !== -1) {
                    db_1.inMemoryStorage.events[eventType].splice(index, 1);
                    deleted = true;
                    break;
                }
            }
     ...
     */
    async getEventCountsByType(startDate, endDate) {
        return (0, db_1.withFallback)(async () => {
            let query = `
        ...
        `;
            ...
            const counts = {};
            for (const eventType in db_1.inMemoryStorage.events) {
                let events = db_1.inMemoryStorage.events[eventType];
                if (startDate) {
