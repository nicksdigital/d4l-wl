...
import "../interfaces/ID4LSoulIdentity.sol";
...
contract D4LERC20Asset is ID4LAsset, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        if (_token == address(0) || _soulManager == address(0)) revert ZeroAddress();
        token = IERC20(_token);
    ...
    function authorizeRoute(bytes32 routeId, bool authorized) external onlyOwner {
        if (routeId == bytes32(0)) revert ZeroValue();
     ...
     */
    function canRoute(bytes32 routeId, address fromSoul, address toSoul, uint256 amount) external view override returns (bool) {
     ...
     */
    function assetType() external pure override returns (uint8) {
        return 1; // ERC20
    }
    ...
    function _getSoulOwner(address soulId) internal view returns (address) {
        ...
        try ID4LSoulIdentity(soulId).owner() returns (address _owner) {
            return _owner;
        } catch {
