...
library D4LTransientStorage {
    ...
    function clearAll() internal {
        // Clear each key individually using the helper functions
        storeBytes32(TS_ROUTE_ID, bytes32(0));
        storeAddress(TS_CALLER, address(0));
        storeUint64(TS_SOURCE_CHAIN_ID, 0);
        storeBytes4(TS_SELECTOR, bytes4(0));
        storeUint8(TS_PERMISSION_LEVEL, 0);
        storeBool(TS_SUCCESS, false);
        storeUint256(TS_GAS_USED, 0);
        storeBytes32(TS_EXECUTION_DATA, bytes32(0));
        storeUint64(TS_DESTINATION_CHAIN_ID, 0);
        storeBytes32(TS_TRANSACTION_ID, bytes32(0));
        storeAddress(TS_BRIDGE_ADAPTER, address(0));
        storeUint256(TS_MESSAGE_FEE, 0);
        storeUint256(TS_GAS_LIMIT, 0);
        storeUint256(TS_REFUND_AMOUNT, 0);
        storeUint8(TS_ROUTE_TYPE, 0);
        storeAddress(TS_IMPLEMENTATION, address(0));
        storeBool(TS_ACTIVE, false);
        storeBytes32(TS_SUPPORTED_CHAINS, bytes32(0));
        storeUint256(TS_CHAIN_COUNT, 0);
        storeBytes32(TS_ROUTE_NAME, bytes32(0));
        storeBytes32(TS_REASON, bytes32(0));
    }
}
