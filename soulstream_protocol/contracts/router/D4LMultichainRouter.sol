...
contract D4LMultichainRouter is ID4LMultichainRouter, AccessControl, ReentrancyGuard, Pausable {
    using D4LTransientStorage for bytes32;
    /// @notice Error thrown when a caller does not have sufficient permissions
    /// @param caller The address of the caller
    /// @param permissionLevel The required permission level
    /// @param reason The reason for the permission failure
    error InsufficientPermissions(address caller, PermissionLevel permissionLevel, string reason);
    ...
    function executeRoute(
        ...
        D4LTransientStorage.storeBytes32(TS_CURRENT_ROUTE_ID, routeId);
        D4LTransientStorage.storeAddress(TS_CURRENT_CALLER, msg.sender);
        D4LTransientStorage.storeBytes4(TS_CURRENT_SELECTOR, route.selector);
        D4LTransientStorage.storeUint256(TS_CURRENT_SOURCE_CHAIN, _currentChainId);
        ...
        D4LTransientStorage.storeUint8(TS_PERMISSION_LEVEL, uint8(permissionLevel));
        ...
        D4LTransientStorage.storeBool(TS_EXECUTION_SUCCESS, success);
    ...
    function receiveCrossChainRoute(
        ...
        // Store route information in transient storage
        D4LTransientStorage.storeBytes32(TS_CURRENT_ROUTE_ID, routeId);
        D4LTransientStorage.storeAddress(TS_CURRENT_CALLER, sender);
        D4LTransientStorage.storeBytes4(TS_CURRENT_SELECTOR, route.selector);
        D4LTransientStorage.storeUint256(TS_CURRENT_SOURCE_CHAIN, sourceChainId);
