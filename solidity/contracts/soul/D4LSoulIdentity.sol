...
contract D4LSoulIdentity is Initializable, ReentrancyGuard {
    ...
    function _executeRoute(
        ...
        if (!success && result.length > 0) {
            // Try to extract the revert reason from the returned data
            // The format is: 0x08c379a0 + 32 bytes offset + 32 bytes length + error string
            // 0x08c379a0 is the function selector for Error(string)
            if (result.length >= 68 &&
                bytes4(result[0:4]) == bytes4(keccak256("Error(string)"))) {
                // Extract the error message
                assembly {
                    result := add(result, 0x04) // Skip the function selector
                }
                // Return the error with context
                revert(string(abi.encodePacked("D4LSoulIdentity: router execution failed: ", result)));
            }
        }
