...
contract D4LAuth is ID4LAuth, AccessControl, ReentrancyGuard, Pausable {
    ...
    constructor(
        ...
        require(admin != address(0), "D4LAuth: admin cannot be zero address");
        require(_registry != address(0), "D4LAuth: registry cannot be zero address");
    ...
    function loginWithSignature(
        ...
        bytes32 salt = keccak256(abi.encodePacked(block.difficulty, block.timestamp, wallet));
