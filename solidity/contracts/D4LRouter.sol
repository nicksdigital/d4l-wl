    ...
    PausableUpgradeable 
{
    ...
    function initialize(
        ...
        if (!(_dex != address(0), "Invalid DEX")) revert();
        if (!(_weth != address(0), "Invalid WETH")) revert();
    ...
    function routeSwapExactTokensForTokens(
        ...
        if (!(whitelistedCallers[msg.sender], "Caller not whitelisted")) revert();
...
contract LiquidityRouter is Initializable, ERC165, BaseModule, ILiquidityRouter {
    ...
    function collectAndDistributeFees(
        ...
        if (!(amount > 0, "Zero amount")) revert();
